          ...
          <div className={`flex justify-between h-14`}>
            <div className="flex items-center">
              <Link href="/" className="overflow-hidden relative inline-block group">
                ...
                <span className="text-xl font-bold text-black dark:text-[#B4B4B4] inline-block transition-transform duration-300 ease-in-out group-hover:translate-x-[-200%]">Shuoan</span>
              </Link>

              <div className={`hidden sm:flex items-center`}>
                <div className="ml-8 flex it items-center gap-4">
                  <Link href="/about" className='relative group text-[#B4B4B4] dark:hover:text-[#4DBBAC] hover:text-[#4DB8AC]'>
                  ...
                  <Link href="/projects" className=' relative group text-[#B4B4B4] dark:hover:text-[#4DBBAC] hover:text-[#4DB8AC]'>
                    <span style={{ height: "1.5px" }} className="absolute w-full  rounded-md bg-[#4DBBAC] bottom-0 left-0 origin-right transform scale-x-0 transition-transform duration-300 ease-in-out  group-hover:origin-left group-hover:scale-x-100"></span>
                    Projects
                  </Link>
                  <Link href="/blog" className='relative group text-[#B4B4B4] dark:hover:text-[#4DBBAC] hover:text-[#4DB8AC]'>
                    <span style={{ height: "1.5px" }} className="absolute w-full  rounded-md bg-[#4DBBAC] bottom-0 left-0 origin-right transform scale-x-0 transition-transform duration-300 ease-in-out  group-hover:origin-left group-hover:scale-x-100"></span>
                    Blog
                  </Link>
                </div>
            ...
            <div className={`flex items-center gap-1`}>
              <Link href='https://www.instagram.com/sa._.climbs' target='_blank' className='hidden sm:flex text-[#B4B4B4] rounded-full hover:shadow-lg hover:text-white p-0.5 hover:bg-gradient-to-br from-[#fdf497] via-[#fd5949] to-[#285AEB] via-[#d6249f]'>
                <InstagramIcon className="items-center h-6 w-6 m-0.5 " />
              </Link>
              <Link href='https://www.linkedin.com/in/lee-shuoan' target='_blank' className='hidden sm:flex text-[#B4B4B4] rounded-full hover:shadow-lg hover:text-white p-0.5 hover:bg-[#0A66C2]'>
                <LinkedInIcon className=" items-center h-6 w-6 m-0.5 " />
              </Link>
              <Link href='https://www.github.com/leeshuoan' target='_blank' className='hidden sm:flex text-[#B4B4B4] rounded-full hover:shadow-lg p-0.5 hover:text-black hover:bg-[#F5F7F9] dark:hover:bg-[#30363D] dark:hover:text-white'>
                <GithubIcon className="items-center h-6 w-6 m-0.5 " />
              </Link>
              <div onClick={toggleDarkMode} className={`${isDarkMode ? 'hover:text-[#D69C11] hover:bg-[#D69C11]' : 'flex hover:text-[#243F6F] hover:bg-[#243F6F]'} rounded-full hover:cursor-pointer hover:shadow-lg p-0.5 text-[#B4B4B4]  hover:bg-opacity-20`}>
